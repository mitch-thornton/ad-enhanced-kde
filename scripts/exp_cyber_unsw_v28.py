#!/usr/bin/env python3
"""exp_cyber_unsw_v28.py -- network-traffic density estimation on UNSW-NB15.

v28 change: the improved Sheather-Jones bandwidth is computed by the direct
Botev-Grotowski-Kroese (2010) fixed point in ad_kde_v31 (embedded, no KDEpy),
replacing the KDEpy improved_sheather_jones call of v27, which was found to
undersmooth systematically (h about 2.6x below the fixed point on smooth targets).
The v27/v30 ISJ column of the cyber table is superseded by this script's output;
all other columns are unchanged in code and should reproduce. Rerun locally with
the same --data-dir and paste back the printed table.
...
USAGE: python3 exp_cyber_unsw_v28.py --data-dir /path/to/unsw [--out fig_cyber.pdf]
"""
...
from ad_kde_v31 import botev_isj_bandwidth

def _silverman_h(d):
    """Exact replication of the KDEpy silvermans_rule used by v27 so the Silverman
    column reproduces: min(sd, IQR/1.3489795) * (3n/4)^(-1/5)."""
    n = len(d)
    if n == 1: return 1.0
