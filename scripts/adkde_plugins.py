...
To use a richer backend, register it before running an experiment:
    ...
    P.register("mixture",       my_mixture_wrapper)        # any density-valued mixture backend
...
reference engine and an external mixture wrapper. Neither is part of the public
image: a practitioner must supply their own blind-group-matching backend and/or group library,
and must obtain any external mixture library under its own license.
"""
import numpy as np
...
# selection (no external dependency). Register an external library for the full cross-family
...
def fit_mixture(d, kmax=6):
    ...
    EM with BIC-selected order. Register a 'mixture' backend for the full
