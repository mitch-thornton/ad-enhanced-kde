#!/usr/bin/env Rscript
# exp_locfit_probe_v1.R -- FOR LOCAL RUN (needs R + locfit: install.packages("locfit"))
# Fair local-likelihood comparison: Loader's locfit with adaptive nearest-neighbor
# bandwidth on Marron-Wand samples, ISE vs truth, same seed lineage.
# Run:  Rscript exp_locfit_probe_v1.R   ; paste the printed table back.
library(locfit)
set.seed(20260627)
...
cat(sprintf("%-16s %6s %12s\n", "target", "n", "locfit ISEx1e3"))
for (tname in names(mw)) {
  ...
  for (n in c(1000, 2000)) {
    ...
    for (r in 1:5) {
      ...
      ises <- c(ises, sum((fh-ftrue)^2)*(xg[2]-xg[1]))
    }
    cat(sprintf("%-16s %6d %12.4f\n", tname, n, 1e3*mean(ises)))
  }
}
