#!/usr/bin/env python3
"""exp_modern_probe_v1.py -- probe-level comparison of AD-KDE against two modern
...
Marron-Wand targets, ISE vs truth. Probe scope: 4 targets, n in {1000},
reps=5, seed lineage 20260627.
"""
...
print("%-16s %10s %10s %10s %10s %10s" % ("target","ISJ","LocalLik","AdaptDiff","AD-Wiener","AD-KDE"))
for tname in TARGETS:
    comps = C.MW[tname]; ftrue = C.mw_pdf(comps, XG)
    acc = {k: [] for k in ("isj","ll","ad","adw","adkde")}
    for r in range(5):
        ...
        acc["ll"].append(ise(local_likelihood(d, XG, 1.5*hI), ftrue))
    ...
    print("%-16s %10.4f %10.4f %10.4f %10.4f %10.4f" % (tname,
        1e3*np.mean(acc["isj"]), 1e3*np.mean(acc["ll"]), 1e3*np.mean(acc["ad"]),
        1e3*np.mean(acc["adw"]), 1e3*np.mean(acc["adkde"])))
print("tool: exp_modern_probe_v1.py ; ISE x1e3 ; n=1000 reps=5 seed0=%d" % SEED0)
